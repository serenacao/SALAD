...
Deno.test("ChallengeParticipationConcept Tests", async (t) => {
  ...
  await t.step(
    "[Action]: acceptInvitation - Successful acceptance",
    async () => {
      ...
      assertEquals(
        foundParticipation?.user,
        userA,
        "Participation user should match"
      );
      assertEquals(
      ...
      assertEquals(
        foundParticipation?.completed,
        false,
        "Participation should not be completed initially"
      );

      const userParticipations = await concept._getUserParticipations({
        user: userA,
      });
      assertEquals(
  ...
  await t.step(
    "[Action]: acceptInvitation - Non-existent invitation",
    async () => {
  ...
  await t.step(
    "[Action]: removeInvitation - Non-existent invitation",
    async () => {
      const nonExistentInvitation = freshID() as ID;
  ...
  await t.step(
    "[Action]: removeParticipation - Successful removal",
    async () => {
      ...
      participationIdC = (acceptPartResult as { participation: ID })
        .participation;
  ...
  await t.step(
    "[Action]: removeParticipation - Non-existent participation",
    async () => {
  ...
  await t.step(
    "[Action]: completeChallenge - Successful completion",
    async () => {
      ...
      participationIdD = (acceptPartResult as { participation: ID })
        .participation;
  ...
  await t.step(
    "[Action]: completeChallenge - Non-existent participation",
    async () => {
      const nonExistentUser = freshID() as ID;
  ...
  await t.step("[Query]: _getChallengeParticipations", async () => {
    ...
    const inv1A = (await concept.createInvitation({
      challenge: challenge1,
      user: userA,
    })) as { invitation: ID };
    ...
    const inv1B = (await concept.createInvitation({
      challenge: challenge1,
      user: userB,
    })) as { invitation: ID };
    ...
    const inv2A = (await concept.createInvitation({
      challenge: challenge2,
      user: userA,
    })) as { invitation: ID };
    ...
    assertEquals(
      c1Participants.length,
      2,
      "Challenge 1 should have 2 participants"
    );
    assertEquals(
    ...
    assertEquals(
      c2Participants.length,
      1,
      "Challenge 2 should have 1 participant"
    );
    assertEquals(
    ...
    assertEquals(
      ...
      0,
      "Non-existent challenge should have 0 participants"
    );
  });
  ...
  await t.step("[Query]: _getChallengeInvitations", async () => {
    ...
    assertEquals(
      ...
      0,
      "Non-existent challenge should have 0 invitees"
    );
  });
  ...
  await t.step("[Query]: _getUserParticipations", async () => {
    ...
    const userAParticipations = await concept._getUserParticipations({
      user: userA,
    });
    assertEquals(
      userAParticipations.length,
      2,
      "User A should have 2 participations"
    );
    assertEquals(
    ...
    assertEquals(
      userBParticipations.length,
      1,
      "User B should have 1 participation"
    );
    assertEquals(
    ...
    const nonExistentUserParticipations = await concept._getUserParticipations({
      user: freshID() as ID,
    });
    assertEquals(
      nonExistentUserParticipations.length,
      0,
      "Non-existent user should have 0 participations"
    );
  });
  ...
  await t.step("[Query]: _getUserInvitations", async () => {
    ...
    assertEquals(
      userAInvitations.length,
      2,
      "User A should have 2 invitations"
    );
    assertEquals(
    ...
    const nonExistentUserInvitations = await concept._getUserInvitations({
      user: freshID() as ID,
    });
    assertEquals(
      nonExistentUserInvitations.length,
      0,
      "Non-existent user should have 0 invitations"
    );
  });

  await t.step(
    "[Trace]: Fulfilling the Principle - User Invitation and Completion",
    async () => {
      ...
      console.log(
        `Step 1: Inviter (${inviter}) invites Invited User (${invitedUser}) to Challenge (${challenge}).`
      );
      const createInvResult = await concept.createInvitation({
        challenge: challenge,
        user: invitedUser,
      ...
      let invitedUserInvitations = await concept._getUserInvitations({
        user: invitedUser,
      });
      assertEquals(
        invitedUserInvitations.length,
        1,
        "Invited user should have 1 invitation"
      );
      assertEquals(
      ...
      console.log(
        `Step 2: Invited User (${invitedUser}) accepts the invitation (${invitationId}).`
      );
      const acceptPartResult = await concept.acceptInvitation({
        invitation: invitationId,
      });
      assertEquals(
        "participation" in acceptPartResult,
        true,
        "Participation should be created successfully"
      );
      const participationId = (acceptPartResult as { participation: ID })
        .participation;
      console.log(`   - Participation ID: ${participationId}`);

      let invitedUserParticipations = await concept._getUserParticipations({
        user: invitedUser,
      });
      assertEquals(
        invitedUserParticipations.length,
        1,
        "Invited user should have 1 participation"
      );
      assertEquals(
      ...
      console.log(
        `   - Verified challenge has the invited user as a participant.`
      );

      let currentParticipationState = await concept["participations"].findOne({
        _id: participationId,
      });
      assertEquals(
        currentParticipationState?.completed,
        false,
        "Participation should initially be incomplete"
      );
      console.log(`   - Verified participation is initially not completed.`);

      console.log(
        `Step 3: Invited User (${invitedUser}) completes the challenge participation (${participationId}).`
      );
      const completeResult = await concept.completeChallenge({
        user: invitedUser,
        challenge: challenge,
      ...
      assertEquals(
        Object.keys(completeResult).length,
        0,
        "Challenge completion should return empty object"
      );
      console.log(`   - Challenge marked as complete.`);

      currentParticipationState = await concept["participations"].findOne({
        _id: participationId,
      });
      assertEquals(
        currentParticipationState?.completed,
