...
Deno.test("Chat Concept", async (t) => {
  ...
  await t.step(
    "should successfully start a chat between two distinct users",
    async () => {
      ...
      const result = await chatConcept.startChat({
        requester: userAlice,
        receiver: userBob,
      });
      ...
      assertExists(
        (result as { chat: ID }).chat,
        "Should return a chat ID on success."
      );
      ...
      assertEquals(
        chatDoc.user1Accessible,
        true,
        "User1Accessible flag should be true."
      );
      assertEquals(
        chatDoc.user2Accessible,
        true,
        "User2Accessible flag should be true."
      );

      // Verify query: chat is retrievable by both users
      const retrievedChat = await chatConcept._getChatBetweenUsers({
        userA: userAlice,
        userB: userBob,
      });
      assertArrayIncludes(
        retrievedChat as Array<{ chat: ID }>,
        [{ chat: chatId }],
        "Chat should be found via query."
      );
  ...
  await t.step("should prevent starting a chat with oneself", async () => {
    ...
    const noChats = await chatConcept.chats
      .find({
  ...
  await t.step(
    "should prevent starting a chat if one already exists between the users",
    async () => {
      ...
      const initialChatResult = await chatConcept.startChat({
        requester: userAlice,
        receiver: userBob,
      });
      ...
      const resultDuplicate = await chatConcept.startChat({
        requester: userAlice,
        receiver: userBob,
      });
      console.log(
        "  startChat result (duplicate, same order):",
        resultDuplicate
      );
      assertExists(
        (resultDuplicate as { error: string }).error,
        "Should return an error for duplicate chat."
      );
      assertEquals(
        (resultDuplicate as { error: string }).error,
      ...
      const resultDuplicateReversed = await chatConcept.startChat({
        requester: userBob,
        receiver: userAlice,
      });
      console.log(
        "  startChat result (duplicate, reversed order):",
        resultDuplicateReversed
      );
      assertExists(
        (resultDuplicateReversed as { error: string }).error,
        "Should return an error for reversed duplicate chat."
      );
      assertEquals(
        (resultDuplicateReversed as { error: string }).error,
        "Chat already exists between these users",
  ...
  await t.step(
    "should successfully send a message in an existing and accessible chat",
    async () => {
      ...
      const chatResult = await chatConcept.startChat({
        requester: userAlice,
        receiver: userBob,
      });
      ...
      const dmResult = await chatConcept.send({
        sender: userAlice,
        receiver: userBob,
      ...
      assertExists(
        (dmResult as { dm: ID }).dm,
        "Should return a DM ID on success."
      );
      ...
      assertEquals(
        dmDoc.chatId,
        chatId,
        "DM should be linked to the correct chat."
      );
      ...
      assertNotEquals(
        (dmsInChat as { error: string }).error,
        "Expected to retrieve DMs, got an error."
      );
      assertExists(
        (dmsInChat as Array<{ dm: object }>)[0],
        "Should find at least one DM."
      );
      assertEquals(
        ((dmsInChat as Array<{ dm: object }>)[0].dm as any).id,
        dmId,
        "Retrieved DM ID should match."
      );
  ...
  await t.step(
    "should start chat and send a message if no chat exists between sender and receiver",
    async () => {
  ...
  await t.step(
    "should delete a chat for one user (making it inaccessible only for them)",
    async () => {
      ...
      const chatId = (chatResult as { chat: ID }).chat;
      assertExists(chatId);
      await chatConcept.send({
        sender: userAlice,
        receiver: userBob,
        message: "Initial message before delete.",
      });

      // Action: Alice deletes the chat for herself
      const deleteResult = await chatConcept.deleteChat({
        chat: chatId,
        user: userAlice,
      });
      ...
      const chatDocAfterDelete = await chatConcept.chats.findOne({
        _id: chatId,
      });
      assertExists(chatDocAfterDelete);
      if (chatDocAfterDelete.user1 === userAlice) {
        assertFalse(
          chatDocAfterDelete.user1Accessible,
          "Alice's accessible flag should be false."
        );
        assertEquals(
          chatDocAfterDelete.user2Accessible,
          true,
          "Bob's accessible flag should remain true."
        );
      } else {
        // Alice is user2
        assertEquals(
          chatDocAfterDelete.user1Accessible,
          true,
          "User1's accessible flag should remain true."
        );
        assertFalse(
          chatDocAfterDelete.user2Accessible,
          "Alice's accessible flag should be false."
      ...
      const aliceAccessibleChats = await chatConcept._getAccessibleChatsForUser(
        { user: userAlice }
      );
      assertFalse(
        (aliceAccessibleChats as Array<{ chat: ID }>).some(
          (c) => c.chat === chatId
        ),
        "Alice should not see the chat."
      );
      // Bob should still see the chat in his accessible list
      const bobAccessibleChats = await chatConcept._getAccessibleChatsForUser({
        user: userBob,
      });
      assertArrayIncludes(
        bobAccessibleChats as Array<{ chat: ID }>,
        [{ chat: chatId }],
        "Bob should still see the chat."
      );
      ...
      assertNotEquals(
        (dmsForChat as { error: string }).error,
        "DMs should still be accessible via query."
      );
      assertEquals(
        (dmsForChat as Array<{ dm: object }>).length,
        1,
        "DM history count should be 1."
      );

      // Action: Alice tries to send another message (should fail)
      const dmFailResult = await chatConcept.send({
        sender: userAlice,
        receiver: userBob,
        message: "Attempt after delete by Alice.",
      });
      assertExists(
        (dmFailResult as { error: string }).error,
        "Alice should not be able to send messages."
      );
      assertEquals(
        (dmFailResult as { error: string }).error,
        "Sender does not have access to this chat",
        "Error message should indicate no access."
      );

      // Action: Bob cannot send messages (even though chat is still accessible to him)
      const dmSuccessResult = await chatConcept.send({
        sender: userBob,
        receiver: userAlice,
        message: "Still here, Alice!",
      });
      console.log(
        "  Bob sending message after Alice deleted:",
        dmSuccessResult
      );
      assertExists(
        (dmSuccessResult as { error: string }).error,
        "Bob should not still be able to send messages."
      );
    }
  );

  await t.step(
    "should completely make a chat inaccessible when both users delete it",
    async () => {
      ...
      const chatDocAfterBothDelete = await chatConcept.chats.findOne({
        _id: chatId,
      });
      assertExists(chatDocAfterBothDelete);
      assertFalse(
        chatDocAfterBothDelete.user1Accessible,
        "User1Accessible flag should be false."
      );
      assertFalse(
        chatDocAfterBothDelete.user2Accessible,
        "User2Accessible flag should be false."
      );
      console.log(
        "  Chat flags after both users deleted:",
        chatDocAfterBothDelete
      );
      ...
      assertFalse(
        (bobAccessibleChats as Array<{ chat: ID }>).some(
          (c) => c.chat === chatId
        ),
        "Bob should not see the chat."
      );
      // DM history should now be inaccessible via query
      const dmsResult = await chatConcept._getDMsInChat({ chat: chatId });
      assertExists(
        (dmsResult as { error: string }).error,
        "DMs should be inaccessible when chat has no active users."
      );
      assertEquals(
        (dmsResult as { error: string }).error,
        "Chat not found or is inaccessible",
        "Error message for inaccessible DMs should match."
      );

      // Action: Attempt to send message by either user should fail
      const dmFailAlice = await chatConcept.send({
        sender: userAlice,
        receiver: userBob,
        message: "Hello again!",
      });
      assertExists(
        (dmFailAlice as { error: string }).error,
        "Alice should not be able to send messages."
      );
      const dmFailBob = await chatConcept.send({
        sender: userBob,
        receiver: userAlice,
        message: "Are you there?",
      });
      assertExists(
        (dmFailBob as { error: string }).error,
        "Bob should not be able to send messages."
      );
    }
  );

  await t.step(
    "should prevent deleting a chat if user is not a participant",
    async () => {
      console.log(
      ...
      console.log(
        "  deleteChat result (non-participant Charlie):",
        deleteResult
      );
      ...
      assertEquals(
        (deleteResult as { error: string }).error,
        "User is not a part of this chat",
        "Error message should match."
      );
      ...
      assertEquals(
        chatDoc.user1Accessible,
        true,
        "User1Accessible flag should still be true."
      );
      assertEquals(
        chatDoc.user2Accessible,
        true,
        "User2Accessible flag should still be true."
      );
    }
  );
  ...
  await t.step(
    "should retrieve all chats accessible to a specific user",
    async () => {
      ...
      const chatABResult = await chatConcept.startChat({
        requester: userAlice,
        receiver: userBob,
      });
      const chatABId = (chatABResult as { chat: ID }).chat;
      const chatACResult = await chatConcept.startChat({
        requester: userAlice,
        receiver: userCharlie,
      });
      const chatACId = (chatACResult as { chat: ID }).chat;
      const chatBCResult = await chatConcept.startChat({
        requester: userBob,
        receiver: userCharlie,
      });
      ...
      const aliceChats = await chatConcept._getAccessibleChatsForUser({
        user: userAlice,
      });
      console.log("  Alice's accessible chats:", aliceChats);
      const aliceChatIds = (aliceChats as Array<{ chat: ID }>).map(
        (c) => c.chat
      );
      assertArrayIncludes(aliceChatIds, [chatACId], "Alice should see chatAC.");
      assertFalse(
        aliceChatIds.includes(chatABId),
        "Alice should not see chatAB (deleted by her)."
      );
      assertFalse(
        aliceChatIds.includes(chatBCId),
        "Alice should not see chatBC (not part of it)."
      );
      assertEquals(
        aliceChatIds.length,
        1,
        "Alice should have 1 accessible chat."
      );

      // Query: Get chats for Bob
      const bobChats = await chatConcept._getAccessibleChatsForUser({
        user: userBob,
      });
      ...
      assertArrayIncludes(
        bobChatIds,
        [chatABId],
        "Bob should see chatAB (Alice deleted, not him)."
      );
      assertFalse(
        bobChatIds.includes(chatACId),
        "Bob should not see chatAC (not part of it)."
      );
      assertFalse(
        bobChatIds.includes(chatBCId),
        "Bob should not see chatBC (deleted by him)."
      );
      assertEquals(bobChatIds.length, 1, "Bob should have 1 accessible chat.");

      // Query: Get chats for Charlie
      const charlieChats = await chatConcept._getAccessibleChatsForUser({
        user: userCharlie,
      });
      console.log("  Charlie's accessible chats:", charlieChats);
      const charlieChatIds = (charlieChats as Array<{ chat: ID }>).map(
        (c) => c.chat
      );
      assertArrayIncludes(
        charlieChatIds,
        [chatACId, chatBCId],
        "Charlie should see chatAC and chatBC."
      );
      assertFalse(
        charlieChatIds.includes(chatABId),
        "Charlie should not see chatAB (not part of it)."
      );
      assertEquals(
        charlieChatIds.length,
        2,
        "Charlie should have 2 accessible chats."
      );
    }
  );

  await t.step(
    "principle: users can start a chat, send messages, and manage chat visibility",
    async () => {
      ...
      console.log(
        "  Scenario: Alice and Bob communicate, then Alice hides the chat, Bob continues, then both hide."
      );
      ...
      assertEquals(
        chatDoc.user1Accessible,
        true,
        "Chat should be accessible to user1 initially."
      );
      assertEquals(
        chatDoc.user2Accessible,
        true,
        "Chat should be accessible to user2 initially."
      );
      ...
      const dmResult1 = await chatConcept.send({
        sender: userAlice,
        receiver: userBob,
        message: message1,
      });
      assertExists(
        (dmResult1 as { dm: ID }).dm,
        "First DM should be sent successfully."
      );
      ...
      const dmResult2 = await chatConcept.send({
        sender: userBob,
        receiver: userAlice,
        message: message2,
      });
      assertExists(
        (dmResult2 as { dm: ID }).dm,
        "Second DM should be sent successfully."
      );
      console.log("    DM 2 sent successfully.");

      // Both users can still see the chat and its messages.
      console.log(
        "  Verifying chat and messages accessibility for both users."
      );
      const initialDMs = await chatConcept._getDMsInChat({ chat: chatId });
      assertNotEquals(
        (initialDMs as { error: string }).error,
        "DMs should be retrievable at this point."
      );
      assertEquals(
        (initialDMs as Array<{ dm: object }>).length,
        2,
        "There should be 2 DMs in the chat."
      );
      console.log(
        "    Current chat history:",
        (initialDMs as Array<any>).map((d) => d.dm.message)
      );
      ...
      if (aliceIsUser1) {
        ...
        assertEquals(
          chatDoc.user2Accessible,
          true,
          "Bob's flag should still be true."
        );
      } else {
        assertEquals(
          chatDoc.user1Accessible,
          true,
          "User1's flag should still be true."
        );
      ...
      console.log(
        "  Step 5: Alice tries to send another message (should fail)."
      );
      ...
      assertEquals(
        (dmFailResult as { error: string }).error,
        "Sender does not have access to this chat",
        "Alice's message attempt failed as expected."
      );
      ...
      assertExists(
        (dmSuccessResult as { error: ID }).error,
        "Bob should not still be able to send messages."
      );
      ...
      const dmsAfterAliceDelete = await chatConcept._getDMsInChat({
        chat: chatId,
      });
      assertNotEquals(
        (dmsAfterAliceDelete as { error: string }).error,
        "DMs should still be visible."
      );
      assertEquals(
        (dmsAfterAliceDelete as Array<{ dm: object }>).length,
        2,
        "There should now be 3 DMs."
      );
      console.log(
        "    Chat history after Alice's deletion (visible via Bob):",
        (dmsAfterAliceDelete as Array<any>).map((d) => d.dm.message)
      );
      ...
      assertExists(
        (dmsFinal as { error: string }).error,
        "DMs should not be retrievable when chat is inaccessible to both."
      );
      assertEquals(
        (dmsFinal as { error: string }).error,
        "Chat not found or is inaccessible",
        "DM retrieval failed as expected."
      );
