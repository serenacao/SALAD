...
Deno.test("Challenges Concept Tests", async (t) => {
  ...
  await t.step(
    "[Action: createChallenge] - Valid challenge creation by user",
    async () => {
      ...
      assertNotEquals(
        ...
        "Should not return an error for valid level."
      );
      assertExists(
        (result as { challenge: ID }).challenge,
        "Should return a challenge ID."
      );
      ...
      assertEquals(
        ...
        1,
        "Challenge details should be retrievable."
      );
      assertEquals(
        challengeDetails[0].exercise,
        "Pushups",
      ...
      assertEquals(
        isOpen[0].result,
        false,
        "Challenge should initially be closed (open: false)."
      );
  ...
  await t.step(
    "[Action: createChallenge] - Invalid parameters (requires check)",
    async () => {
      ...
      console.log(
        `Confirmed 'createChallenge' enforces its 'requires' conditions.`
      );
    }
  );
  ...
  await t.step(
    "[Action: openChallenge] - Open an existing challenge",
    async () => {
      console.log(`\n--- Test: openChallenge ---`);
  ...
  await t.step(
    "[Action: openChallenge] - Try to open a non-existent challenge (requires check)",
    async () => {
      ...
      assertEquals(
        ...
        "Should return error for non-existent challenge."
      );
      console.log(
        `Rejected opening non-existent challenge: ${JSON.stringify(result)}`
      );
    }
  );
  ...
  await t.step(
    "[Action: closeChallenge] - Close an existing challenge",
    async () => {
      console.log(`\n--- Test: closeChallenge ---`);
      let isOpen = await concept._isOpen({ challenge: challenge1Id });
      assertEquals(
        isOpen[0].result,
        true,
        "Challenge should currently be open."
      );
  ...
  await t.step(
    "[Action: closeChallenge] - Try to close a non-existent challenge (requires check)",
    async () => {
      ...
      console.log(
        `Rejected closing non-existent challenge: ${JSON.stringify(result)}`
      );
    }
  );
  ...
  // --- Action: inviteUsers ---
  await t.step(
    "[Action: inviteUsers] - Invite users to a challenge",
    async () => {
      console.log(`\n--- Test: inviteUsers ---`);
      const result = await concept.inviteUsers({
      ...
      assertEquals(
        invitees.map((i) => i.user).sort(),
        [userB, userC].sort(),
        "Both users should be invited."
      );
      ...
      assertEquals(
        ...
        false,
        "User B should be invited but not yet accepted/participant."
      );
      console.log(
        `Confirmed users are added to invitees with accepted: false.`
      );
    }
  );

  await t.step(
    "[Action: inviteUsers] - Invite same users again (no duplicate entries)",
    async () => {
      console.log(`\n--- Test: inviteUsers (duplicates) ---`);
      const result = await concept.inviteUsers({
        challenge: challenge1Id,
        users: [userB],
      });
      ...
      assertEquals(
        invitees.map((i) => i.user).sort(),
        [userB, userC].sort(),
        "Should not add duplicate invitees."
      );
      console.log(
        `Confirmed inviting same user again does not create duplicates.`
      );
    }
  );

  await t.step(
    "[Action: inviteUsers] - Invite to non-existent challenge (requires check)",
    async () => {
      console.log(`\n--- Test: inviteUsers (non-existent) ---`);
      const nonExistentId = freshID();
      const result = await concept.inviteUsers({
      ...
      console.log(
        `Rejected inviting to non-existent challenge: ${JSON.stringify(result)}`
      );
    }
  );
  ...
  await t.step(
    "[Action: acceptChallenge] - User accepts challenge",
    async () => {
      ...
      assertEquals(
        isParticipantB[0].result,
        true,
        "User B should now be an accepted participant."
      );

      const isParticipantC = await concept._isParticipant({
        challenge: challenge1Id,
        user: userC,
      });
      assertEquals(
        isParticipantC[0].result,
        false,
        "User C should still not be an accepted participant."
      );
  ...
  await t.step(
    "[Action: acceptChallenge] - User not invited to challenge (requires check)",
    async () => {
      ...
      assertEquals(
        result,
        { error: "User is not invited to this challenge." },
        "Should reject non-invited user."
      );
      console.log(
        `Rejected accepting by non-invited user: ${JSON.stringify(result)}`
      );
    }
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      assertEquals(
        ...
        "Should reject non-participating user."
      );
      console.log(
        `Rejected leaving by non-participating user: ${JSON.stringify(result)}`
      );
    }
  );
  ...
  await t.step("[Action: completePart] - User completes a part", async () => {
    ...
    const isCompletedPart = await concept._isCompletedPart({
      part: firstPartId,
      user: userB,
    });
    assertEquals(
      ...
      true,
      "User B should have completed the first part."
    );

    const isCompletedChallenge = await concept._isCompletedChallenge({
      challenge: challenge1Id,
      user: userB,
    });
    assertEquals(
      ...
      false,
      "User B should not have completed the whole challenge yet."
    );
    console.log(`Confirmed user B completed a single part.`);
  });

  await t.step(
    "[Action: completePart] - User completes all parts, challenge completed",
    async () => {
      ...
      for (let i = 1; i < allParts.length; i++) {
        ...
        await concept.completePart({
          part: allParts[i]._id.toString() as ID,
          user: userB,
        });
      }
      ...
      for (const part of allParts) {
        const isCompleted = await concept._isCompletedPart({
          part: part._id.toString() as ID,
          user: userB,
        });
        assertEquals(
          isCompleted[0].result,
          true,
          `User B should have completed part ${part._id}.`
        );
      }
      ...
      assertEquals(
        isCompletedChallenge[0].result,
        true,
        "User B should have completed the entire challenge."
      );
      console.log(
        `Confirmed user B completed all parts and the entire challenge.`
      );
    }
  );

  await t.step(
    "[Action: completePart] - Challenge not open (requires check)",
    async () => {
      ...
      const result = await concept.completePart({
        part: firstPartId.toString() as ID,
        user: userB,
      });
      assertEquals(
        result,
        { error: "Challenge is not open." },
        "Should reject completion if challenge is closed."
      );
      console.log(
        `Rejected completing part in a closed challenge: ${JSON.stringify(
          result
        )}`
      );
  ...
  await t.step(
    "[Action: completePart] - User not accepted participant (requires check)",
    async () => {
      ...
      assertEquals(
        ...
        "Should reject non-accepted participant."
      );
      console.log(
        `Rejected completing part by non-accepted participant: ${JSON.stringify(
          result
        )}`
      );
    }
  ...
  await t.step(
    "[Action: createVerificationRequest] - Create a valid verification request",
    async () => {
      ...
      console.log(`Action result: ${JSON.stringify(result)}`);
      assertExists(
        (result as { verificationRequest: ID }).verificationRequest,
        "Should return a verification request ID."
      );
  ...
  await t.step(
    "[Action: createVerificationRequest] - Requester is same as approver (requires check)",
    async () => {
      console.log(
      ...
      assertEquals(
        result,
        { error: "Requester must be distinct from Approver." },
        "Should reject requester == approver."
      );
      console.log(
        `Rejected verification request where requester equals approver: ${JSON.stringify(
          result
        )}`
      );
    }
  ...
  await t.step(
    ...
    async () => {
      ...
      assertEquals(
        ...
        "Should reject verifying an already approved request."
      );
      console.log(
        `Rejected verifying already approved request: ${JSON.stringify(
          alreadyApprovedResult
        )}`
      );
      ...
      assertEquals(
        ...
        "Should reject verifying non-existent request."
      );
      console.log(
        `Rejected verifying non-existent request: ${JSON.stringify(
          nonExistentResult
        )}`
      );
    }
  ...
  await t.step(
    ...
    async () => {
      ...
      const initialChallengeDetails = await concept._getChallengeDetails({
        challenge: principleChallengeId,
      });
      assertEquals(
        initialChallengeDetails[0].exercise,
        "Jump Ropes",
        "Principle challenge created correctly."
      );
      assertEquals(
        (await concept._isOpen({ challenge: principleChallengeId }))[0].result,
        false,
        "Principle challenge starts closed."
      );
      ...
      assertEquals(
        (await concept._isOpen({ challenge: principleChallengeId }))[0].result,
        true,
        "Principle challenge is now open."
      );

      // 3. User A invites User B and User C to the challenge.
      await concept.inviteUsers({
        challenge: principleChallengeId,
        users: [userB, userC],
      });
      console.log(`3. User A invited User B and User C.`);
      assertEquals(
        (await concept._getInvitees({ challenge: principleChallengeId }))
          .map((i) => i.user)
          .sort(),
        [userB, userC].sort(),
        "User B and C invited."
      );
      ...
      const principleParts = await db
        ...
        .toArray();
      assertEquals(
        principleParts.length,
        1,
        "Principle challenge should have 1 part."
      );
      ...
      assertEquals(
        (
          await concept._isCompletedPart({
            part: principlePartId.toString() as ID,
            user: userB,
          })
        )[0].result,
        true,
        "User B completed the part."
      );

      // 6. This triggers completion of the entire challenge for User B.
      assertEquals(
        (
          await concept._isCompletedChallenge({
            challenge: principleChallengeId,
            user: userB,
          })
        )[0].result,
        true,
        "User B completed the entire challenge."
      );
      ...
      const principleVrId = (vrRes as { verificationRequest: ID })
        .verificationRequest;
      console.log(
      ...
      assertEquals(
        ...
        false,
        "Verification request is initially not approved."
      );
      ...
      assertEquals(
        ...
        true,
        "Verification request is now approved."
      );

      // 9. User C tries to complete a part (should fail as not accepted).
      const userCFailsToComplete = await concept.completePart({
        part: principlePartId.toString() as ID,
        user: userC,
      });
      assertEquals(
        userCFailsToComplete,
        { error: "User is not an accepted participant in this challenge." },
        "User C cannot complete a part because not accepted."
      );
      console.log(
      ...
      assertEquals(
        (
          await concept._isInvited({
            challenge: principleChallengeId,
            user: userB,
          })
        )[0].result,
        false,
        "User B is no longer invited/participating."
      );

      console.log(
        `Principle trace complete. All steps align with expected behavior and state changes.`
      );
    }
  );
  ...
  await t.step(
    "[Action: deleteChallenge] - Delete an existing challenge and its associated data",
    async () => {
      ...
      const challengeToDeleteId = (createResToDelete as { challenge: ID })
        .challenge;
      ...
      const verificationRequest = await concept.createVerificationRequest({
        part: partToDeleteId.toString() as ID,
        requester: userB,
        approver: userA,
        evidence: testFileId,
      });
      ...
      console.log(
        `Confirmed 'deleteChallenge' removes the challenge and all associated parts and verification requests.`
      );
    }
  );

  await t.step(
    "[Action: deleteChallenge] - Try to delete a non-existent challenge (requires check)",
    async () => {
      ...
      console.log(
        `Rejected deleting non-existent challenge: ${JSON.stringify(result)}`
      );
    }
  );
